...
const mutations = {
  INIT_VIEW: (state, views) => {
    state.visitedViews = views
    state.cachedViews = views
    state.lastVisitedView = {}
  },
  ADD_VISITED_VIEW: (state, view) => {
    if (state.visitedViews.some(v => v.path === view.path)) return
    const lastViewIndex = state.visitedViews.findIndex(v => v.path === state.lastVisitedView.path)
  ...
  ADD_LAST_VISITED_VIEW: (state, view) => {
    if (view.path === '/') return
    state.lastVisitedView = view
  },
