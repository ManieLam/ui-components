...
.anso-table
  AnsoDatatable(
    ...
    //-   div 左
    //- template(v-slot:filter-right)
    //-   div 右
    //- template(v-slot:filter-footer)
    //-   div 下
    //- template(v-slot:filter-info-left)
    //-   div 左2
    //- template(v-slot:filter-info-right)
    //-   div 右2
    //- template(v-slot:toolbar-left)
    //-   div 左3
    //- template(v-slot:toolbar-right)
    //-   div 右3
    //- template(#prep-header)
    //-   div 上2
    //- template(#prep-footer)
    //-   div 下2
  el-drawer.drawer(
    :title="formTitle"
    size="50%"
...
export default {
  ...
  methods: {
    ...
    refreshTable () {
      this.$nextTick(() => {
        this.$refs.synthesisTable.reRenderTableData()
      })
    },
    handleClose () {
      this.drawer = false
    ...
    editData (form) {
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          const formData = cloneDeep(form.formData)
          const index = this.tableData.findIndex(data => data.id === formData.id)
    ...
    addData (form) {
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          const formData = cloneDeep(form.formData)
          const maxId = Math.max.apply(Math, this.tableData.map(item => { return item.id }))
          formData.id = maxId + 1
